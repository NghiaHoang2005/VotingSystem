...
export default function VotePage() {
  ...
  const handleVote = async (choice: number) => {
    ...
    // Encrypt the choice (1 for YES, 0 for NO)
      ...
      {!voteCasted ? (
        <div className="flex gap-4 justify-center">
          <button 
            ...
            disabled={loading}
            className="btn-primary py-4 px-8 text-2xl font-bold rounded-xl bg-green-600 hover:bg-green-500 shadow-[0_0_15px_rgba(34,197,94,0.5)]"
          >
            ĐỒNG Ý (YES)
          </button>
          <button 
            ...
            disabled={loading}
            className="btn-secondary py-4 px-8 text-2xl font-bold rounded-xl bg-red-900/50 hover:bg-red-800 border-red-500/50"
          >
            TỪ CHỐI (NO)
          </button>
        </div>
        ...
        <div className="bg-slate-900/50 p-6 rounded-xl border border-green-500 text-left">
          ...
          <p className="text-sm text-slate-400 mb-2">Trình duyệt đã mã hóa phiếu của bạn. Máy chủ chỉ nhận được chuỗi dữ liệu sau (Hoàn toàn không biết bạn bầu YES hay NO):</p>
          <div className="bg-slate-950 p-4 rounded text-xs text-slate-500 font-mono break-all max-h-40 overflow-y-auto">
            {lastCiphertext}
          </div>
